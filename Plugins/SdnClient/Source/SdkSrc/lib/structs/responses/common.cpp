...
using namespace sdn::events::account_data;
...
{
    response.event_id = obj.at("event_id").get<sdn::identifiers::Event>();
}
...
{
    sdn::utils::log::log()->warn("Error parsing events: {}, {}", err.what(), event.dump(2));
}

void
log_error(const std::string &err, const json &event)
{
    sdn::utils::log::log()->warn("Error parsing events: {}, {}", err, event.dump(2));
}

void
parse_room_account_data_events(
  const json &events,
  std::vector<sdn::events::collections::RoomAccountDataEvents> &container)
{
    container.clear();
    container.reserve(events.size());

    for (const auto &e : events) {
        const auto type = sdn::events::getEventType(e);
...
compose_timeline_events(json &events,
                        const std::vector<sdn::events::collections::TimelineEvents> &container)
{
...
parse_timeline_events(const json &events,
                      std::vector<sdn::events::collections::TimelineEvents> &container)
{
    container.clear();
    container.reserve(events.size());
    ...
    for (const auto &e : events) {
        ...
        try {
            ...
            case events::EventType::RoomEncrypted: {
                container.emplace_back(events::EncryptedEvent<sdn::events::msg::Encrypted>(e));
                break;
            }
            case events::EventType::RoomEncryption: {
            ...
            case events::EventType::RoomRedaction: {
                container.emplace_back(events::RedactionEvent<sdn::events::msg::Redaction>(e));
            ...
            case events::EventType::RoomMessage: {
                ...
                const auto msg_type = sdn::events::getMessageType(e.at("content"));
...
parse_device_events(const json &events,
                    std::vector<sdn::events::collections::DeviceEvents> &container)
{
    container.clear();
    container.reserve(events.size());
    for (const auto &e : events) {
...
parse_state_events(const json &events,
                   std::vector<sdn::events::collections::StateEvents> &container)
{
    container.clear();
    container.reserve(events.size());
...
parse_stripped_events(const json &events,
                      std::vector<sdn::events::collections::StrippedEvents> &container)
{
    container.clear();
    container.reserve(events.size());
    ...
    for (const auto &e : events) {
        ...
        try {
            switch (type) {
            case events::EventType::RoomAliases: {
                container.emplace_back(events::StrippedEvent<sdn::events::state::Aliases>(e));
...
parse_ephemeral_events(const json &events,
                       std::vector<sdn::events::collections::EphemeralEvents> &container)
{
    container.clear();
    container.reserve(events.size());
    for (const auto &e : events) {
