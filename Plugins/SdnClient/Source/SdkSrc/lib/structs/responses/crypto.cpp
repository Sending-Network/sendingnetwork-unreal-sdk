...
from_json(const nlohmann::json &obj, QueryKeys &response)
{
    ...
    if (obj.contains("master_keys"))
        response.master_keys =
          obj.at("master_keys").get<std::map<std::string, sdn::crypto::CrossSigningKeys>>();
    if (obj.contains("user_signing_keys"))
        response.user_signing_keys =
          obj.at("user_signing_keys").get<std::map<std::string, sdn::crypto::CrossSigningKeys>>();
    if (obj.contains("self_signing_keys"))
        response.self_signing_keys =
          obj.at("self_signing_keys").get<std::map<std::string, sdn::crypto::CrossSigningKeys>>();
}
