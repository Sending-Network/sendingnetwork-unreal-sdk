...
#include "sdn/responses/public_rooms.hpp"
...
{
    res.visibility = sdn::common::stringToVisibility(obj.at("visibility").get<std::string>());
}
...
from_json(const nlohmann::json &obj, PublicRoomsChunk &res)
{
    ...
    res.join_rule = sdn::events::state::stringToJoinRule(obj.value("join_rule", "public"));
    ...
    res.membership = sdn::events::state::stringToMembership(
    ...
    if (obj.contains("children_state"))
        sdn::responses::utils::parse_stripped_events(obj.at("children_state"), res.children_state);
}
