#include "sdn/responses/sync.hpp"
...
{
    using Name   = sdn::events::StrippedEvent<sdn::events::state::Name>;
    using Member = sdn::events::StrippedEvent<sdn::events::state::Member>;
...
{
    using Avatar = sdn::events::StrippedEvent<sdn::events::state::Avatar>;
    using Member = sdn::events::StrippedEvent<sdn::events::state::Member>;
    ...
    if (auto entries = obj.find("join"); entries != obj.end()) {
        for (const auto &r : entries->items()) {
            if (r.key().size() < 256) {
                ...
                sdn::utils::log::log()->warn("Skipping roomid which exceeds 255 bytes.");
            }
        }
    }
...
from_json(const json &obj, DeviceLists &device_lists)
{
    if (obj.count("changed") != 0) {
        ...
        std::erase_if(device_lists.changed, [](const std::string &user) {
            if (user.size() > 255) {
                sdn::utils::log::log()->warn("Invalid userid in device list changed.");
                return true;
            } else
                return false;
        });
    ...
    if (obj.count("left") != 0) {
        ...
        std::erase_if(device_lists.left, [](const std::string &user) {
            if (user.size() > 255) {
                sdn::utils::log::log()->warn("Invalid userid in device list left.");
                return true;
            } else
                return false;
        });
        ...
        for (const auto &e : events) {
            try {
                response.presence.push_back(
                  e.get<sdn::events::Event<sdn::events::presence::Presence>>());
            } catch (std::exception &ex) {
                sdn::utils::log::log()->warn(
