...
to_json(nlohmann::json &obj, const PushCondition &condition)
{
    ...
    if (!condition.is.empty())
        obj["is"] = condition.is;
    if (condition.rel_type != sdn::common::RelationType::Unsupported)
...
from_json(const nlohmann::json &obj, PushCondition &condition)
{
    ...
    condition.rel_type         = obj.value("rel_type", sdn::common::RelationType::Unsupported);
...
struct PushRuleEvaluator::OptimizedRules
{
    ...
    struct OptimizedRule
    {
        ...
        struct RelatedEventCondition
        {
            ...
            sdn::common::RelationType rel_type = sdn::common::RelationType::Unsupported;
        ...
        [[nodiscard]] bool matches(
          ...
          const std::map<sdn::common::RelationType, RelatedEvents> &relatedEventsFlat) const
        {
    ...
    auto add_conditions_to_rule = [](OptimizedRules::OptimizedRule &rule,
        ...
        for (const auto &cond : conditions) {
            if (cond.kind == "event_match") {
                ...
                if (cond.rel_type != sdn::common::RelationType::Unsupported) {
                ...
                sdn::utils::log::log()->info("Skipping rule with unknown condition type: {}",
                                             cond.kind);
                return false;
            }
        }
...
PushRuleEvaluator::evaluate(
  const sdn::events::collections::TimelineEvents &event,
  const RoomContext &ctx,
  const std::vector<std::pair<sdn::common::Relation, sdn::events::collections::TimelineEvents>>
    &relatedEvents) const
{
    auto event_json = nlohmann::json(event);
    auto flat_event = flatten_event(event_json);

    std::map<sdn::common::RelationType, RelatedEvents> relatedEventsFlat;
    for (const auto &[rel, ev] : relatedEvents) {
        if (rel.rel_type != sdn::common::RelationType::Unsupported) {
            if (rel.is_fallback)
