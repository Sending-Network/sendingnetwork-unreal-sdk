...
to_json(json &obj, const Login &request)
{
    ...
    if (!request.random_server.empty())
        obj["random_server"] = request.random_server;

    if (!request.updated.empty())
        obj["updated"] = request.updated;
    ...
    std::visit(
      [&obj](const auto &id) {
          ...
          } else if constexpr (std::is_same_v<const login_identifier::DID &,
                                              decltype(id)>) {
              obj["identifier"]["did"]     = id.did;
              obj["identifier"]["address"] = id.address;
              obj["identifier"]["message"] = id.message;
              obj["identifier"]["token"]   = id.token;
              obj["identifier"]["app_token"]   = id.app_token;
          }
      },
      request.identifier);
...
void
to_json(json &obj, const PreLogin &request)
{
    obj["did"] = request.did;
...
{
    obj["visibility"] = sdn::common::visibilityToString(request.visibility);
}
