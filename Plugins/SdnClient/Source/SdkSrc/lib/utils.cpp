...
sdn::client::utils::MxcUrl
sdn::client::utils::parse_mxc_url(const std::string &url)
{
...
bool
sdn::client::utils::is_number(const std::string &s)
{
    return !s.empty() &&
...
std::string
sdn::client::utils::random_token(uint8_t len, bool with_symbols) noexcept
{
    std::string symbols = "!@#$%^&*()";
...
std::string
sdn::client::utils::query_params(const std::map<std::string, std::string> &params) noexcept
{
    if (params.empty())
        return "";
...
std::string
sdn::client::utils::url_encode(const std::string &value) noexcept
{
