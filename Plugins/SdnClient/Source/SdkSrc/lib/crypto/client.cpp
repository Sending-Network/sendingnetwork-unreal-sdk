...
#include "sdnclient/crypto/utils.hpp"
...
OlmClient::one_time_keys()
{
    ...
    return json::parse(std::string(buf.begin(), buf.end())).get<sdn::crypto::OneTimeKeys>();
}
...
OlmClient::unpublished_fallback_keys()
{
    ...
    return json::parse(std::string(fbuf.begin(), fbuf.end())).get<sdn::crypto::OneTimeKeys>();
}
...
std::map<std::string, sdn::requests::SignedOneTimeKey>
OlmClient::sign_one_time_keys(const OneTimeKeys &keys, bool fallback)
{
    // Sign & append the one time keys.
    std::map<std::string, sdn::requests::SignedOneTimeKey> signed_one_time_keys;
...
sdn::requests::UploadKeys
OlmClient::create_upload_keys_request(const sdn::crypto::OneTimeKeys &one_time_keys,
                                      const sdn::crypto::OneTimeKeys &fallback_keys)
{
...
OlmClient::create_ssss_key(const std::string &password)
{
    ...
    if (password.empty()) {
        ...
        sdn::secret_storage::PBKDF2 pbkdf2{};
        ...
        setup.privateKey = sdn::crypto::PBKDF2_HMAC_SHA_512(
...
std::string
sdn::crypto::session_id(OlmSession *s)
{
...
std::string
sdn::crypto::session_id(OlmOutboundGroupSession *s)
{
...
std::string
sdn::crypto::session_key(OlmOutboundGroupSession *s)
{
...
std::string
sdn::crypto::export_session(OlmInboundGroupSession *s, uint32_t at_index)
{
...
InboundGroupSessionPtr
sdn::crypto::import_session(const std::string &session_key)
{
    auto session = create_olm_object<InboundSessionObject>();
...
bool
sdn::crypto::matches_inbound_session(OlmSession *session, const std::string &one_time_key_message)
{
    auto tmp = create_buffer(one_time_key_message.size());
    std::copy(one_time_key_message.begin(), one_time_key_message.end(), tmp.begin());
...
bool
sdn::crypto::matches_inbound_session_from(OlmSession *session,
...
bool
sdn::crypto::verify_identity_signature(const DeviceKeys &device_keys,
                                       ...
                                       const UserId &user_id)
{
    try {
        ...
        sdn::utils::log::log()->error("verify_identity_signature: {}", e.what());
    }
...
bool
sdn::crypto::ed25519_verify_signature(std::string signing_key,
                                      nlohmann::json obj,
                                      std::string signature)
{
    try {
        ...
        sdn::utils::log::log()->error("verify_signature: {}", e.what());
    }
...
std::string
sdn::crypto::encrypt_exported_sessions(const sdn::crypto::ExportedSessionKeys &keys,
                                       const std::string &pass)
{
    ...
    buf                 = sdn::crypto::PBKDF2_HMAC_SHA_512(pass, salt, iterations);

    BinaryBuf aes256 = BinaryBuf(buf.begin(), buf.begin() + 32);

    BinaryBuf hmac256 = BinaryBuf(buf.begin() + 32, buf.begin() + (2UL * 32));

    auto ciphertext = sdn::crypto::AES_CTR_256_Encrypt(plaintext, aes256, nonce);

    uint8_t iterationsArr[4];
    sdn::crypto::uint32_to_uint8(iterationsArr, iterations);
    ...
    auto hmacSha256 = sdn::crypto::HMAC_SHA256(hmac256, output);
...
sdn::crypto::ExportedSessionKeys
sdn::crypto::decrypt_exported_sessions(const std::string &data, const std::string &pass)
{
    // Parse the data into a base64 string without the header and footer
    std::string unpacked = sdn::crypto::unpack_key_file(data);
    ...
    uint32_t rounds;
    sdn::crypto::uint8_to_uint32(rounds_arr, rounds);
    ...
    // derive the keys
    auto buf = sdn::crypto::PBKDF2_HMAC_SHA_512(pass, salt, rounds);
    ...
    auto hmacSha256 = sdn::crypto::HMAC_SHA256(hmac256, BinaryBuf(binary_start, json_end));
    ...
    auto decrypted = sdn::crypto::AES_CTR_256_Decrypt(ciphertext, aes256, iv);

    std::string plaintext(decrypted.begin(), decrypted.end());
    return json::parse(plaintext).get<sdn::crypto::ExportedSessionKeys>();
}
