...
key_from_passphrase(const std::string &password,
                    const sdn::secret_storage::AesHmacSha2KeyDescription &parameters)
{
...
std::string
decrypt(const sdn::secret_storage::AesHmacSha2EncryptedData &data,
...
sdn::secret_storage::AesHmacSha2EncryptedData
encrypt(const std::string &data, const BinaryBuf &decryptionKey, const std::string &key_name)
{
    sdn::secret_storage::AesHmacSha2EncryptedData encrypted{};
...
sdn::responses::backup::EncryptedSessionData
encrypt_session(const sdn::responses::backup::SessionData &data, const std::string &publicKey)
{
    sdn::responses::backup::EncryptedSessionData d;
...
sdn::responses::backup::SessionData
decrypt_session(const sdn::responses::backup::EncryptedSessionData &data,
                const BinaryBuf &privateKey)
{
    return nlohmann::json::parse(
             CURVE25519_AES_SHA2_Decrypt(data.ciphertext, privateKey, data.ephemeral, data.mac))
      .get<sdn::responses::backup::SessionData>();
}
...
BinaryBuf
decrypt_file(const std::string &ciphertext, const sdn::crypto::EncryptedFile &encryption_info)
{
...
std::pair<BinaryBuf, sdn::crypto::EncryptedFile>
encrypt_file(const std::string &plaintext)
{
    sdn::crypto::EncryptedFile encryption_info;
