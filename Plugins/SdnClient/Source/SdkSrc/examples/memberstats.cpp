...
using Memberlist = decltype(sdn::responses::Members::chunk);

using Memberevent = decltype(sdn::responses::Members::chunk)::value_type;

struct HistoryEvent
{
    std::string user;
    uint64_t ts = 0;
    sdn::events::state::Membership membership;
};
...
member_fetch_loop(Memberevent current)
{
    while (current.unsigned_data.replaces_state.empty()) {
        if (member_to_ts.empty() || member_to_ts.back().user != current.state_key ||
            ...
            if (current.content.membership == sdn::events::state::Membership::Join) {
                member_to_ts.push_back(
                  {current.state_key, current.origin_server_ts, current.content.membership});
                std::cerr << ",";
            } else if (current.content.membership == sdn::events::state::Membership::Leave) {
                member_to_ts.push_back(
                  {current.state_key, current.origin_server_ts, current.content.membership});
                std::cerr << ";";
            }
    ...
    client->get_event(
      ...
      [](sdn::events::collections::TimelineEvents ev, RequestErr err) mutable {
          if (err) {
...
login_handler(const sdn::responses::Login &res, RequestErr err)
{
    ...
    client->members(roomid, [](sdn::responses::Members members_, RequestErr err) {
        if (err) {
...
main()
{
    ...
    for (const auto &e : member_to_ts)
        file << e.ts << "\t" << (e.membership == sdn::events::state::Membership::Join ? "j" : "l")
             << "\t" << e.user << "\n";
    file.close();
