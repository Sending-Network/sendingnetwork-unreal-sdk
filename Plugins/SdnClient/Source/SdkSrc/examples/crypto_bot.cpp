...
void
save_device_keys(const sdn::responses::QueryKeys &res);

void
mark_encrypted_room(const RoomId &id);

void
handle_to_device_msgs(const sdn::responses::ToDevice &to_device);
    ...
    void save()
    {
        ...
        for (const auto &s : olm_inbound_sessions)
            data["olm_inbound_sessions"][s.first] =
              sdn::crypto::pickle<SessionObject>(s.second.get(), STORAGE_KEY);
        ...
        for (const auto &s : inbound_group_sessions)
            data["inbound_group_sessions"][s.first] =
              sdn::crypto::pickle<InboundSessionObject>(s.second.get(), STORAGE_KEY);

        for (const auto &s : outbound_group_sessions)
            data["outbound_group_sessions"][s.first] =
              sdn::crypto::pickle<OutboundSessionObject>(s.second.get(), STORAGE_KEY);
...
print_errors(RequestErr err)
{
    ...
    if (!err->sdn_error.error.empty())
        console->error("sdn error: {}", err->sdn_error.error);
    if (err->error_code)
                   ...
                   const std::string &msg)
{
    ...
    client->send_room_message<msg::Encrypted>(
      room_id, data, [](const sdn::responses::EventId &res, RequestErr err) {
          if (err) {
              print_errors(err);
              return;
          }
...
create_outbound_megolm_session(const std::string &room_id, const std::string &reply_msg)
{
    ...
    const auto session_id  = sdn::crypto::session_id(outbound_session.get());
    const auto session_key = sdn::crypto::session_key(outbound_session.get());

    sdn::events::DeviceEvent<sdn::events::msg::RoomKey> megolm_payload;
    ...
    megolm_payload.type                = sdn::events::EventType::RoomKey;
...
void
keys_uploaded_cb(const sdn::responses::UploadKeys &, RequestErr err)
{
    if (err) {
        print_errors(err);
        return;
...
void
parse_messages(const sdn::responses::Sync &res)
{
    for (const auto &room : res.rooms.invite) {
        auto room_id = room.first;

        console->info("joining room {}", room_id);
        client->join_room(room_id, [room_id](const sdn::responses::RoomId &, RequestErr e) {
            if (e) {
                print_errors(e);
    ...
    for (const auto &room : res.rooms.join) {
        ...
        for (const auto &e : room.second.state.events) {
            if (is_room_encryption(e)) {
                ...
                auto m = std::get<sdn::events::StateEvent<sdn::events::state::Member>>(e);
...
get_device_keys(const UserId &user)
{
    ...
    client->query_keys(query, [](const sdn::responses::QueryKeys &res, RequestErr err) {
        if (err) {
            print_errors(err);
            return;
        }
        ...
        for (const auto &key : res.device_keys) {
            ...
            for (const auto &device : devices) {
                ...
                try {
                    auto ok =
                      verify_identity_signature(nlohmann::json(data).get<sdn::crypto::DeviceKeys>(),
                                                DeviceId(id),
                                                UserId(user_id));
...
void
handle_to_device_msgs(const sdn::responses::ToDevice &msgs)
{
...
void
login_cb(const sdn::responses::Login &, RequestErr err)
{
    if (err) {
    ...
    client->upload_keys(olm_client->create_upload_keys_request(),
                        [](const sdn::responses::UploadKeys &, RequestErr err) {
                            if (err) {
                                print_errors(err);
                                return;
                            }
...
void
join_room_cb(const sdn::responses::RoomId &, RequestErr err)
{
    if (err) {
        print_errors(err);
        return;
