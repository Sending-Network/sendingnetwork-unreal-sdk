...
static void
export_backup(const sdn::secret_storage::AesHmacSha2KeyDescription &keyDesc,
              const sdn::secret_storage::AesHmacSha2EncryptedData &secretData,
              const sdn::responses::backup::KeysBackup &backup);
...
void
login_handler(const sdn::responses::Login &res, RequestErr err)
{
    if (err) {
        cerr << "There was an error during login: " << err->sdn_error.error << "\n";
        return;
    }
    ...
    client->backup_version(
      [](const sdn::responses::backup::BackupVersion &backup_version, RequestErr err) {
          if (err) {
              cerr << "Error fetching the backup version: ";
              print_errors(err);
              client->logout([](sdn::responses::Logout, RequestErr) {});
              return;
          }

          if (backup_version.algorithm != sdn::responses::backup::megolm_backup_v1) {
              cerr << "Incompatible backup algorithm: " << backup_version.algorithm << "\n";
              client->logout([](sdn::responses::Logout, RequestErr) {});
              return;
          }
          client->room_keys(
            backup_version.version, [](sdn::responses::backup::KeysBackup backup, RequestErr err) {
                if (err) {
                ...
                client->secret_storage_secret(
                  sdn::secret_storage::secrets::megolm_backup_v1,
                  [backup](sdn::secret_storage::Secret secret, RequestErr err) {
                      if (err) {
                      ...
                      client->secret_storage_key(
                        ...
                        [backup, secretData = secret.encrypted.begin()->second](
                          sdn::secret_storage::AesHmacSha2KeyDescription keyDesc, RequestErr err) {
                            client->logout([](sdn::responses::Logout, RequestErr) {});
                            if (err) {
...
export_backup(const sdn::secret_storage::AesHmacSha2KeyDescription &keyDesc,
              ...
              const sdn::responses::backup::KeysBackup &backup)
{
    sdn::crypto::BinaryBuf decryptionKey;
    if (keyDesc.passphrase) {
        std::optional<sdn::crypto::BinaryBuf> temp;
        do {
            auto password = getpass("Enter Key Backup Password: ");
            temp          = sdn::crypto::key_from_passphrase(password, keyDesc);
        } while (!temp);
        ...
        do {
            ...
            temp             = sdn::crypto::key_from_recoverykey(recoveryKey, keyDesc);
        } while (!temp);
        decryptionKey = temp.value();
    }
    ...
    auto decryptedSecret =
      decrypt(secretData, decryptionKey, sdn::secret_storage::secrets::megolm_backup_v1);
    ...
    for (const auto &[room_id, backup_sessions] : backup.rooms) {
        for (const auto &[session_id, s] : backup_sessions.sessions) {
            try {
                ...
                sdn::crypto::ExportedSession export_session{};
    ...
    auto encrypted_file = sdn::crypto::encrypt_exported_sessions(
      {exported_sessions}, getpass("Encryption password for export file:"));

    std::ofstream file;
    file.open("exported_sessions.txt");
    file << HEADER_LINE << "\n" << sdn::crypto::bin2base64(encrypted_file) << "\n" << TRAILER_LINE;
