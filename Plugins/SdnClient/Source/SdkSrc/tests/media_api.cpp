...
void
validate_upload(const sdn::responses::ContentURI &res, RequestErr err)
{
    if (err) {
        if (err->status_code)
...
TEST(MediaAPI, UploadTextFile)
{
    ...
    alice->login("alice", "secret", [alice](const sdn::responses::Login &, RequestErr err) {
        ...
        alice->upload(text,
                      "text/plain",
                      "doc.txt",
                      [alice, text](const sdn::responses::ContentURI &res, RequestErr err) {
...
TEST(MediaAPI, UploadAudio)
{
    ...
    bob->login("bob", "secret", [bob](const sdn::responses::Login &, RequestErr err) {
        ...
        bob->upload(audio,
                    "audio/mp3",
                    "sound.mp3",
                    [bob, audio](const sdn::responses::ContentURI &res, RequestErr err) {
                        validate_upload(res, err);
...
TEST(MediaAPI, UploadImage)
{
    ...
    carl->login("carl", "secret", [carl](const sdn::responses::Login &, RequestErr err) {
        ...
        carl->upload(img,
                     ...
                     [carl, img](const sdn::responses::ContentURI &res, RequestErr err) {
                         validate_upload(res, err);
                     });
