...
TEST(Requests, Login)
{
    ...
    t1.identifier                  = login_identifier::User{"@alice:sdn.org"};
    t1.password                    = "secret";
    t1.initial_device_display_name = "Mobile";

    json j = t1;
    ASSERT_EQ(j.dump(),
              "{\"identifier\":{\"type\":\"m.id.user\",\"user\":\"@alice:sdn.org\"},"
              "\"initial_device_display_name\":\"Mobile\",\"password\":\"secret\",\"type\":"
              "\"m.login.password\"}");

    t2.identifier = login_identifier::User{"@bob:sdn.org"};
    t2.password   = "secret2";

    j = t2;
    ASSERT_EQ(j.dump(),
              "{\"identifier\":{\"type\":\"m.id.user\",\"user\":\"@bob:sdn.org\"},"
              "\"password\":\"secret2\",\"type\":\"m.login.password\"}");

    t3.identifier = login_identifier::User{"@carl:sdn.org"};
    t3.password   = "secret3";
    ...
    ASSERT_EQ(j.dump(),
              ...
              "@carl:sdn.org\"},\"password\":\"secret3\",\"type\":\"m.login."
              "password\"}");
}
...
TEST(Requests, UploadKeys)
{
    ...
    sdn::requests::SignedOneTimeKey k1;
    ...
    sdn::requests::SignedOneTimeKey k2;
...
{
    using namespace sdn::user_interactive;
    // Auth pw{
    //  .session = "<session ID>",
  ...
  "session": "<session ID>"
    ...
    a.content =
      auth::EmailIdentity{"<identity server session id>",
                          ...
                          "<url of identity server authed with, e.g. 'sdn.org:8090'>",
  ...
  "threepid_creds": {
    ...
    "id_server": "<url of identity server authed with, e.g. 'sdn.org:8090'>",
...
{
    sdn::requests::PublicRoomVisibility req;
    req.visibility = sdn::common::RoomVisibility::Private;
    json j         = req;
    EXPECT_EQ(j, R"({
    "visibility" : "private"
  })"_json);
...
TEST(Requests, PublicRooms)
{
    ...
    // default to false
    b2.limit                   = 10;
    b2.third_party_instance_id = "sdn";
    j                          = b2;
    EXPECT_EQ(j, R"({
    "limit" : 10,
    "include_all_networks" : false,
    "third_party_instance_id" : "sdn"
  })"_json);
