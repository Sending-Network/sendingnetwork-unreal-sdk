...
TEST(ExportSessions, EncryptDecrypt)
{
    ...
    std::string ciphertext = sdn::crypto::encrypt_exported_sessions(keys, PASS);
    EXPECT_TRUE(ciphertext.size() > 0);

    auto encoded = bin2base64(ciphertext);

    auto restored_keys = sdn::crypto::decrypt_exported_sessions(encoded, PASS);
...
TEST(Encryption, EncryptedFile)
{
    ...
    ASSERT_EQ(16, sdn::crypto::base642bin_unpadded(encryption_data.second.iv).size());
    EXPECT_EQ(sdn::crypto::bin2base64_unpadded(std::string(8, '\0')),
              encryption_data.second.iv.substr(11));
    auto iv = sdn::crypto::base642bin_unpadded(encryption_data.second.iv);
...
{
    sdn::responses::backup::SessionData d;
    d.algorithm   = "m.megolm.v1.aes-sha2";
    d.sender_key  = "sender_key";
    d.session_key = "session_key";

    auto privKey = sdn::crypto::create_buffer(256 / 8);

    auto publicKey = sdn::crypto::CURVE25519_public_key_from_private(privKey);
    auto encrypted = sdn::crypto::encrypt_session(d, publicKey);
    auto decrypted = sdn::crypto::decrypt_session(encrypted, privKey);
...
TEST(SecretStorage, Secret)
{
    ...
    sdn::secret_storage::Secret secret = j.get<sdn::secret_storage::Secret>();
...
TEST(SecretStorage, SecretKey)
{
    ...
    sdn::secret_storage::AesHmacSha2KeyDescription desc =
      j.get<sdn::secret_storage::AesHmacSha2KeyDescription>();
    ...
    desc = j.get<sdn::secret_storage::AesHmacSha2KeyDescription>();
...
{
    auto ssss1 = sdn::crypto::OlmClient::create_ssss_key();
    ...
    auto ssss2 = sdn::crypto::OlmClient::create_ssss_key("some passphrase");
    ...
    EXPECT_EQ(sdn::crypto::key_from_passphrase("some passphrase", ssss2->keyDescription),
              ssss2->privateKey);
}

TEST(SecretStorage, CreateOnlineKeyBackup)
{
    sdn::crypto::OlmClient account;
    ...
    sdn::responses::backup::SessionData s;
    s.algorithm   = sdn::crypto::MEGOLM_ALGO;
    s.sender_key  = "abc";
    s.session_key = "cde";

    auto enc1 = sdn::crypto::encrypt_session(
      s, json::parse(okb->backupVersion.auth_data)["public_key"].get<std::string>());
    EXPECT_FALSE(enc1.ciphertext.empty());

    auto enc2 = sdn::crypto::encrypt_session(
      s, sdn::crypto::CURVE25519_public_key_from_private(okb->privateKey));
