...
{
    const std::string user_id   = "@alice:sdn.org";
    ...
    ASSERT_TRUE(verify_identity_signature(
      body.get<sdn::crypto::DeviceKeys>(), DeviceId(device_id), UserId(user_id)));
    ASSERT_TRUE(verify_identity_signature(
      device_keys.get<sdn::crypto::DeviceKeys>(), DeviceId(device_id), UserId(user_id)));
}
...
TEST(Utilities, VerifySignature)
{
    ...
    sdn::crypto::CrossSigningKeys msg;
    ...
    EXPECT_EQ(sdn::crypto::ed25519_verify_signature(alice->identity_keys().ed25519, j1, sig), true);
    EXPECT_EQ(
      sdn::crypto::ed25519_verify_signature(alice->identity_keys().ed25519, json(msg), sig1), true);
}
...
TEST(Utilities, VerifyIdentityKeyJson)
{
    ...
    ASSERT_TRUE(verify_identity_signature(
      data.get<sdn::crypto::DeviceKeys>(), DeviceId(device_id), UserId(user_id)));
}
