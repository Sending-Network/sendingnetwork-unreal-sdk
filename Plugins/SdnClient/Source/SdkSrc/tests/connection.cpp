...
TEST(Basic, Connection)
{
    ...
    client->versions([](const sdn::responses::Versions &, RequestErr err) { check_error(err); });
    client->close();
}
...
TEST(Basic, ServerWithPort)
{
    ...
    alice->versions([](const sdn::responses::Versions &, RequestErr err) { check_error(err); });
    alice->close();
}
...
TEST(Basic, Failure)
{
    ...
    alice->verify_certificates(false);
    alice->versions([](const sdn::responses::Versions &, RequestErr err) { ASSERT_TRUE(err); });
    alice->close();
}
...
TEST(Basic, Shutdown)
{
    ...
    client->login("carl", "secret", [client](const sdn::responses::Login &, RequestErr err) {
        check_error(err);
    });
    ...
    client->sync(opts, [client, &opts](const sdn::responses::Sync &res, RequestErr err) {
        check_error(err);

        opts.since   = res.next_batch;
        opts.timeout = 60'000; // milliseconds
        client->sync(opts, [](const sdn::responses::Sync &, RequestErr) {});
    });
...
TEST(Basic, ExceptionInHandler)
{
    ...
    int counter = 0;
    client->versions([&counter](const sdn::responses::Versions &, RequestErr) {
        counter++;
