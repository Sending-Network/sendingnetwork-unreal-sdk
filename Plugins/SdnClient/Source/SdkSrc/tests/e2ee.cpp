...
sdn::requests::UploadKeys
generate_keys(std::shared_ptr<sdn::crypto::OlmClient> account)
{
...
TEST(Encryption, UploadIdentityKeys)
{
    ...
    auto olm_account = std::make_shared<sdn::crypto::OlmClient>();
    ...
    sdn::crypto::OneTimeKeys unused;
    auto request = olm_account->create_upload_keys_request(unused, {});

    // Make the request with the signed identity keys.
    alice->upload_keys(request, [](const sdn::responses::UploadKeys &res, RequestErr err) {
        check_error(err);
...
TEST(Encryption, UploadOneTimeKeys)
{
    ...
    sdn::requests::UploadKeys req = olm_account->create_upload_keys_request();

    alice->upload_keys(req, [](const sdn::responses::UploadKeys &res, RequestErr err) {
        check_error(err);
...
TEST(Encryption, UploadSignedOneTimeKeys)
{
    ...
    sdn::requests::UploadKeys req;
    for (const auto &[key_id, key] : olm_account->sign_one_time_keys(one_time_keys))
        req.one_time_keys[key_id] = key;

    alice->upload_keys(req, [nkeys](const sdn::responses::UploadKeys &res, RequestErr err) {
        check_error(err);
...
{
    auto alice     = make_test_client();
    auto alice_olm = std::make_shared<sdn::crypto::OlmClient>();

    auto bob     = make_test_client();
    auto bob_olm = std::make_shared<sdn::crypto::OlmClient>();
    ...
    alice->upload_keys(alice_req,
                       [&uploads](const sdn::responses::UploadKeys &res, RequestErr err) {
                           check_error(err);
                           EXPECT_EQ(res.one_time_key_counts.size(), 1);
                           EXPECT_EQ(res.one_time_key_counts.at("signed_curve25519"), 1);
    ...
    bob->upload_keys(bob_req, [&uploads](const sdn::responses::UploadKeys &res, RequestErr err) {
        check_error(err);
        EXPECT_EQ(res.one_time_key_counts.size(), 1);
        EXPECT_EQ(res.one_time_key_counts.at("signed_curve25519"), 1);
    ...
    sdn::requests::QueryKeys alice_rk;
    alice_rk.device_keys[bob->user_id().to_string()] = {};
    alice->query_keys(
      alice_rk, [&responses, bob, bob_req](const sdn::responses::QueryKeys &res, RequestErr err) {
          check_error(err);
    ...
    sdn::requests::QueryKeys bob_rk;
    bob_rk.device_keys[alice->user_id().to_string()] = {};
    bob->query_keys(
      bob_rk, [&responses, alice, alice_req](const sdn::responses::QueryKeys &res, RequestErr err) {
          check_error(err);
...
TEST(Encryption, ClaimKeys)
{
    ...
    bob->upload_keys(bob_olm->create_upload_keys_request(),
                     [&uploaded](const sdn::responses::UploadKeys &res, RequestErr err) {
                         check_error(err);
                         EXPECT_EQ(res.one_time_key_counts.size(), 1);
                         EXPECT_EQ(res.one_time_key_counts.at("signed_curve25519"), 1);
                         uploaded = true;
    ...
    alice_rk.device_keys[bob->user_id().to_string()] = {};
    alice->query_keys(
      alice_rk, [alice_olm, alice, bob](const sdn::responses::QueryKeys &res, RequestErr err) {
          check_error(err);
          ...
          // Verify signature.
          ASSERT_TRUE(verify_identity_signature(json(current_device).get<sdn::crypto::DeviceKeys>(),
                                                DeviceId(bob->device_id()),
                                                UserId(bob->user_id().to_string())));

          sdn::requests::ClaimKeys claim_keys;
          for (const auto &d : devices)
              claim_keys.one_time_keys[bob->user_id().to_string()][d] = SIGNED_CURVE25519;
          alice->claim_keys(
            claim_keys,
            [alice_olm, bob, bob_ed25519](const sdn::responses::ClaimKeys &res, RequestErr err) {
                check_error(err);
...
TEST(Encryption, ClaimMultipleDeviceKeys)
{
    ...
    atomic_int uploads(0);
    auto upload_cb = [&uploads](const sdn::responses::UploadKeys &res, RequestErr err) {
        check_error(err);
        EXPECT_EQ(res.one_time_key_counts.size(), 1);
    ...
    bob->claim_keys(claim_keys,
                    [user_id = alice1->user_id().to_string()](const sdn::responses::ClaimKeys &res,
                                                              RequestErr err) {
                        check_error(err);
...
TEST(Encryption, UploadCrossSigningKeys)
{
    ...
    // Make the request with the signed identity keys.
    alice->upload_keys(
      request, [](const sdn::responses::UploadKeys &, RequestErr err) { check_error(err); });

    auto xsign_keys = olm_account->create_crosssigning_keys();
    ASSERT_TRUE(xsign_keys.has_value());
    sdn::requests::DeviceSigningUpload u;
    ...
    alice->device_signing_upload(
      ...
      [xsign_keys, olm_account, alice](RequestErr e) {
          ...
          alice->post_backup_version(bk->backupVersion.algorithm,
                                     ...
                                     [](const sdn::responses::Version &v, RequestErr e) {
                                         check_error(e);
...
TEST(Encryption, KeyChanges)
{
    ...
    auto carl_olm = std::make_shared<sdn::crypto::OlmClient>();
    carl_olm->create_new_account();

    carl->login(
      "carl", "secret", [](const sdn::responses::Login &, RequestErr err) { check_error(err); });
    ...
    carl->create_room(req, [carl, carl_olm](const sdn::responses::CreateRoom &, RequestErr err) {
        check_error(err);

        // Carl syncs to get the first next_batch token.
        SyncOpts opts;
        opts.timeout = 0;
        carl->sync(opts, [carl, carl_olm](const sdn::responses::Sync &res, RequestErr err) {
            check_error(err);
            ...
            carl->upload_keys(
              key_req,
              [carl, next_batch_token](const sdn::responses::UploadKeys &, RequestErr err) {
                  check_error(err);
                  ...
                  carl->key_changes(next_batch_token,
                                    "",
                                    [carl](const sdn::responses::KeyChanges &res, RequestErr err) {
                                        check_error(err);
...
TEST(Encryption, EnableEncryption)
{
    ...
    atomic_int responses(0);
    sdn::identifiers::Room joined_room;

    sdn::requests::CreateRoom req;
    req.invite = {"@carl:" + server_name()};
    bob->create_room(
      req,
      [bob, carl, &responses, &joined_room](const sdn::responses::CreateRoom &res, RequestErr err) {
          check_error(err);
          joined_room = res.room_id;

          bob->enable_encryption(res.room_id.to_string(),
                                 [&responses](const sdn::responses::EventId &, RequestErr err) {
                                     check_error(err);
                                     responses += 1;
                                 });

          carl->join_room(res.room_id.to_string(),
                          [&responses](const sdn::responses::RoomId &, RequestErr err) {
                              check_error(err);
                              responses += 1;
                          });
      });
...
{
    auto alice = make_shared<sdn::crypto::OlmClient>();
    auto bob   = make_shared<sdn::crypto::OlmClient>();
    ...
    auto session_id = sdn::crypto::session_id(outbound_session.get());
    ASSERT_FALSE(session_id.empty());
    auto session_key = sdn::crypto::session_key(outbound_session.get());
...
TEST(Encryption, MegolmSessions)
{
    ...
    const auto session_id  = sdn::crypto::session_id(outbound_megolm_session.get());
    const auto session_key = sdn::crypto::session_key(outbound_megolm_session.get());
...
TEST(Encryption, OlmRoomKeyEncryption)
{
    ...
    alice_http->query_keys(query,
                           [&request_finished, &bob_ed25519, &bob_curve25519, bob = bob_http](
                             const sdn::responses::QueryKeys &res, RequestErr err) {
                               check_error(err);
    ...
    alice_http->claim_keys(claim_keys,
                           [&bob_otk, bob = bob_http, &request_finished](
                             const sdn::responses::ClaimKeys &res, RequestErr err) {
                               check_error(err);
    ...
    opts.timeout = 0;
    bob_http->sync(
      opts, [bob = bob_olm, SECRET_TEXT](const sdn::responses::Sync &res, RequestErr err) {
          check_error(err);

          EXPECT_EQ(res.to_device.events.size(), 1);

          auto olm_msg =
            std::get<DeviceEvent<sdn::events::msg::OlmEncrypted>>(res.to_device.events[0]).content;
...
TEST(Encryption, ShareSecret)
{
    ...
    // Alice create m.room.key request
    sdn::events::msg::SecretRequest secretRequest{};
    ...
    bob_http->send_to_device<SecretRequest>(
      ...
      [&](RequestErr err) {
          ...
          alice_http->sync(opts, [&](const sdn::responses::Sync &res, RequestErr err) {
              check_error(err);

              sdn::events::DeviceEvent<sdn::events::msg::SecretSend> secretSend{};
              secretSend.content.secret = SECRET_TEXT;
              secretSend.content.request_id =
                std::get<sdn::events::DeviceEvent<sdn::events::msg::SecretRequest>>(
                  res.to_device.events.at(0))
                  .content.request_id;
              secretSend.type = sdn::events::EventType::SecretSend;
              ...
              std::map<sdn::identifiers::User,
                       std::map<std::string, sdn::events::msg::OlmEncrypted>>
                body{{bob_http->user_id(),
                      {{bob_http->device_id(), device_msg.get<sdn::events::msg::OlmEncrypted>()}}}};
    ...
    bob_http->sync(
      opts,
      [bob = bob_olm, SECRET_TEXT, REQUEST_ID](const sdn::responses::Sync &res, RequestErr err) {
          check_error(err);
          ...
          auto ev = json::parse(std::string_view((char *)output.data(), output.size()))
                      .get<sdn::events::DeviceEvent<sdn::events::msg::SecretSend>>();
...
TEST(Encryption, PickleMegolmSessions)
{
    ...
    const auto original_session_id  = sdn::crypto::session_id(outbound_session.get());
    const auto original_session_key = sdn::crypto::session_key(outbound_session.get());
    ...
    const auto restored_session_id  = sdn::crypto::session_id(restored_outbound_session.get());
    const auto restored_session_key = sdn::crypto::session_key(restored_outbound_session.get());
...
{
    {
        auto buffer = sdn::crypto::create_buffer(16);
        ASSERT_EQ(buffer.size(), 16);
        auto buf_str = sdn::crypto::to_string(buffer);
        ASSERT_EQ(buf_str.size(), 16);
    }

    std::string plaintext = "This is some plain text payload";
    auto encryption_data  = sdn::crypto::encrypt_file(plaintext);
    ASSERT_NE(plaintext, sdn::crypto::to_string(encryption_data.first));
    ASSERT_EQ(plaintext,
              sdn::crypto::to_string(sdn::crypto::decrypt_file(
                sdn::crypto::to_string(encryption_data.first), encryption_data.second)));
    // key needs to be 32 bytes/256 bits
    ASSERT_EQ(32, sdn::crypto::base642bin_urlsafe_unpadded(encryption_data.second.key.k).size());
    // IV needs to be 16 bytes/128 bits
    ASSERT_EQ(16, sdn::crypto::base642bin_unpadded(encryption_data.second.iv).size());
 ...
 "event_id": "$1575320135447DEPky:neko.dev",
  ...
  "room_id": "!YnUlhwgbBaGcAFsJOJ:neko.dev"
    ...
    sdn::events::RoomEvent<sdn::events::msg::File> ev =
      j.get<sdn::events::RoomEvent<sdn::events::msg::File>>();

    ASSERT_EQ("abcdefg\n",
              sdn::crypto::to_string(
                sdn::crypto::decrypt_file("=\xFDX\xAB\xCA\xEB\x8F\xFF", ev.content.file.value())));
}
