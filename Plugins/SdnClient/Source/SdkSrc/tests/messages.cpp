...
TEST(RoomEvents, Reaction)
{
  ...
  "event_id": "$ujXAq1WXebS-vcpA4yBIZPvCeqGvnrMFP1c1qn8_wJump"
    ...
    EXPECT_EQ(event.content.relations.relations.at(0).rel_type,
              sdn::common::RelationType::Annotation);
...
TEST(RoomEvents, Redacted)
{
    json data = R"({
	  "unsigned": {
            "age": 146,
	    "redacted_by": "$152135702813129HltcO:sdn.org"
	  },
          "content": {},
    ...
    EXPECT_EQ(event.unsigned_data.redacted_by, "$152135702813129HltcO:sdn.org");
}
...
TEST(RoomEvents, AudioMessage)
{
    ...
    EXPECT_EQ(event.content.relations.relations.at(0).rel_type,
              sdn::common::RelationType::InReplyTo);
}
...
TEST(RoomEvents, ElementEffectMessage)
{
    ...
    EXPECT_EQ(event.type, EventType::RoomMessage);
    EXPECT_EQ(event.event_id, "$15104893562785758wEgEU:sdn.org");
...
TEST(RoomEvents, EmoteMessage)
{
    json data = R"({
          ...
          "sender": "@mujx:sdn.org",
          "event_id": "$15098786822025533uttji:sdn.org",
          "unsigned": {
            "age": 626351821
          },
          "content": {
    ...
    EXPECT_EQ(event.type, EventType::RoomMessage);
    EXPECT_EQ(event.event_id, "$15098786822025533uttji:sdn.org");
...
TEST(RoomEvents, FileMessage)
{
    json data = R"({
          ...
          "sender": "@nheko_test:sdn.org",
          "event_id": "$15104856072749611ERqhw:sdn.org",
          "unsigned": {
            "age": 31,
	      ...
	      "thumbnail_url": "mxc://sdn.org/XpxykZBESCSQnYkLKbbIKnVn"
            },
            "msgtype": "m.file",
            "url": "mxc://sdn.org/XpxykZBESCSQnYkLKbbIKnVn",
	      "m.relates_to": {
		  "m.in_reply_to": {
                       "event_id": "$6GKhAfJOcwNd69lgSizdcTob8z2pWQgBOZPrnsWMA1E"
                  }
    ...
    EXPECT_EQ(event.type, EventType::RoomMessage);
    EXPECT_EQ(event.event_id, "$15104856072749611ERqhw:sdn.org");
    EXPECT_EQ(event.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event.sender, "@nheko_test:sdn.org");
    ...
    EXPECT_EQ(event.content.url, "mxc://sdn.org/XpxykZBESCSQnYkLKbbIKnVn");
...
TEST(RoomEvents, ImageMessage)
{
    json data = R"({
          ...
          "type": "m.room.message",
          "room_id": "!cURbafjkfsMDVwdRDQ:sdn.org"
        })"_json;
    ...
    EXPECT_EQ(event.room_id, "!cURbafjkfsMDVwdRDQ:sdn.org");
...
TEST(RoomEvents, NoticeMessage)
{
    json data = R"({
          ...
          "sender": "@_neb_github:sdn.org",
          "event_id": "$15104358652239178iCnZy:sdn.org",
          "unsigned": {
            "age": 69168455
          },
          "content": {
          ...
          "type": "m.room.message",
          "room_id": "!BPvgRcBVHzyFSlYkrg:sdn.org"
        })"_json;

    RoomEvent<msg::Notice> event = data.get<RoomEvent<msg::Notice>>();

    EXPECT_EQ(event.type, EventType::RoomMessage);
    EXPECT_EQ(event.event_id, "$15104358652239178iCnZy:sdn.org");
    EXPECT_EQ(event.room_id, "!BPvgRcBVHzyFSlYkrg:sdn.org");
    EXPECT_EQ(event.sender, "@_neb_github:sdn.org");
...
TEST(RoomEvents, Sticker)
{
    ...
    EXPECT_EQ(event.content.url, "mxc://sdn.org/sHhqkFCvSkFwtmvtETOtKnLP");
...
TEST(FormattedMessages, Deserialization)
{
    ...
    EXPECT_EQ(notice.content.format, "org.sdn.custom.html");
    ...
    EXPECT_EQ(text.content.format, "org.sdn.custom.html");
    ...
    EXPECT_EQ(emote.content.format, "org.sdn.custom.html");
