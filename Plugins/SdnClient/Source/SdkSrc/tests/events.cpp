...
TEST(Events, Redaction)
{
    json data = R"({
	  "unsigned": {
          ...
          "redacts": "$1521361675759563UDexf:sdn.org",
    ...
    EXPECT_EQ(event.redacts, "$1521361675759563UDexf:sdn.org");
...
TEST(Events, Redacted)
{
    ...
    sdn::events::collections::TimelineEvents event =
      data.get<sdn::events::collections::TimelineEvents>();
    ...
    event = data2.get<sdn::events::collections::TimelineEvents>();
...
TEST(StateEvents, Avatar)
{
    json data = R"({
          ...
          "sender": "@mujx:sdn.org",
          "event_id": "$15067620711415511reUFC:sdn.org",
          "age": 3717700323,
          "unsigned": {
            "age": 3717700323
          },
          "state_key": "",
          "content": {
            "url": "mxc://sdn.org/EPKcIsAPzREMrvZIjrIuJnEl"
          },
          "room_id": "!VaMCVKSVcyPtXbcMXh:sdn.org",
          "user_id": "@mujx:sdn.org",
          "type": "m.room.avatar"
        })"_json;
    ...
    EXPECT_EQ(event.event_id, "$15067620711415511reUFC:sdn.org");
    EXPECT_EQ(event.room_id, "!VaMCVKSVcyPtXbcMXh:sdn.org");
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    EXPECT_EQ(event.unsigned_data.age, 3717700323);
    EXPECT_EQ(event.origin_server_ts, 1506762071625L);
    EXPECT_EQ(event.state_key, "");
    EXPECT_EQ(event.content.url, "mxc://sdn.org/EPKcIsAPzREMrvZIjrIuJnEl");
}
...
TEST(StateEvents, Create)
{
    json data = R"({
          ...
          "sender": "@mujx:sdn.org",
          "event_id": "$15067619231414398jhvQC:sdn.org",
          "unsigned": {
            "age": 3715756343
          },
          "state_key": "",
          "content": {
            "creator": "@mujx:sdn.org"
          },
          "type": "m.room.create"
        })"_json;
    ...
    EXPECT_EQ(event.event_id, "$15067619231414398jhvQC:sdn.org");
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    EXPECT_EQ(event.unsigned_data.age, 3715756343L);
    EXPECT_EQ(event.origin_server_ts, 1506761923948L);
    EXPECT_EQ(event.state_key, "");
    // EXPECT_EQ(event.content.creator, "@mujx:sdn.org");
...
TEST(StateEvents, GuestAccess)
{
    ...
    EXPECT_EQ(event.origin_server_ts, 1506761923948L);
    EXPECT_EQ(event.state_key, "");
    EXPECT_EQ(event.content.guest_access, sdn::events::state::AccessState::CanJoin);
}
...
TEST(StateEvents, HistoryVisibility)
{
    json data = R"({
	  ...
	  "sender": "@nheko_test:sdn.org",
	  "event_id": "$15104731332646268uOFJp:sdn.org",
	  "unsigned": {
	    "age": 140
	  },
	  "state_key": "",
	  ...
	  "room_id": "!lfoDRlNFWlvOnvkBwQ:sdn.org"
	})"_json;
    ...
    EXPECT_EQ(event.event_id, "$15104731332646268uOFJp:sdn.org");
    EXPECT_EQ(event.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event.sender, "@nheko_test:sdn.org");
    ...
    json data2 = R"({
          ...
          "sender": "@nheko_test:sdn.org",
          "event_id": "$15104767782674661tXoeB:sdn.org",
          "unsigned": {
            "prev_content": {
              "history_visibility": "shared"
            },
            "prev_sender": "@nheko_test:sdn.org",
            "replaces_state": "$15104731332646268uOFJp:sdn.org",
            "age": 48
          },
          "state_key": "",
          "content": {
    ...
    EXPECT_EQ(event2.event_id, "$15104767782674661tXoeB:sdn.org");
    EXPECT_EQ(event2.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event2.sender, "@nheko_test:sdn.org");
    EXPECT_EQ(event2.origin_server_ts, 1510476778190L);
    EXPECT_EQ(event2.unsigned_data.age, 48);
    EXPECT_EQ(event2.unsigned_data.replaces_state, "$15104731332646268uOFJp:sdn.org");
...
TEST(StateEvents, JoinRules)
{
    json data = R"({
          ...
          "sender": "@mujx:sdn.org",
          "event_id": "$15067619241414401ASocy:sdn.org",
          "unsigned": {
            "age": 3715756273
	  },
          "state_key": "",
    ...
    EXPECT_EQ(event.event_id, "$15067619241414401ASocy:sdn.org");
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    ...
    ASSERT_EQ(event2.content.allow[0].type, sdn::events::state::JoinAllowanceType::RoomMembership);
    ASSERT_EQ(event2.content.allow[0].room_id, "!mods:example.org");
    ASSERT_EQ(event2.content.allow[1].type, sdn::events::state::JoinAllowanceType::RoomMembership);
...
TEST(StateEvents, Member)
{
    json data = R"({
          ...
          "sender": "@nheko_test:sdn.org",
          "event_id": "$15104731322646264oUPqj:sdn.org",
          "unsigned": {
            "age": 72
          },
          "state_key": "@nheko_test:sdn.org",
          "content": {
            "membership": "join",
            "avatar_url": "mxc://sdn.org/JKiSOBDDxCHxmaLAgoQwSAHa",
    ...
    EXPECT_EQ(event.event_id, "$15104731322646264oUPqj:sdn.org");
    EXPECT_EQ(event.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event.sender, "@nheko_test:sdn.org");
    EXPECT_EQ(event.origin_server_ts, 1510473132947L);
    EXPECT_EQ(event.unsigned_data.age, 72);
    EXPECT_EQ(event.state_key, "@nheko_test:sdn.org");
    EXPECT_EQ(event.content.membership, ns::state::Membership::Join);
    EXPECT_EQ(event.content.display_name, "NhekoTest");
    EXPECT_EQ(event.content.avatar_url, "mxc://sdn.org/JKiSOBDDxCHxmaLAgoQwSAHa");

    json data2 = R"({
          "prev_content": {
            "membership": "join",
            "avatar_url": "mxc://sdn.org/IvqcnGakfvwwKeZxbJWhblFp",
            "displayname": "NhekoTest"
          },
          "origin_server_ts": 1509214100884,
          "sender": "@nheko_test:sdn.org",
          "event_id": "$15092141005099019aHvYG:sdn.org",
          "age": 1259000688,
          "unsigned": {
            "prev_content": {
              "membership": "join",
            ...
            "prev_sender": "@nheko_test:sdn.org",
            "replaces_state": "$15092124385075888YpYOh:sdn.org",
            "age": 1259000688
          },
          "state_key": "@nheko_test:sdn.org",
          "content": {
    ...
    EXPECT_EQ(event2.event_id, "$15092141005099019aHvYG:sdn.org");
    EXPECT_EQ(event2.room_id, "!VaMCVKSVcyPtXbcMXh:sdn.org");
    EXPECT_EQ(event2.sender, "@nheko_test:sdn.org");
    EXPECT_EQ(event2.origin_server_ts, 1509214100884L);
    EXPECT_EQ(event2.unsigned_data.replaces_state, "$15092124385075888YpYOh:sdn.org");
    EXPECT_EQ(event2.unsigned_data.age, 1259000688);
    EXPECT_EQ(event2.unsigned_data.age, 1259000688);
    EXPECT_EQ(event2.state_key, "@nheko_test:sdn.org");
    EXPECT_EQ(event2.content.membership, ns::state::Membership::Join);
    EXPECT_EQ(event2.content.display_name, "NhekoTest");
    EXPECT_EQ(event2.content.avatar_url, "mxc://sdn.org/JKiSOBDDxCHxmaLAgoQwSAHa");
}
...
TEST(StateEvents, Name)
{
    json data = R"({
          ...
          "sender": "@nheko_test:sdn.org",
          "event_id": "$15104731332646270uaKBS:sdn.org",
          "unsigned": {
            "age": 70
          },
          "state_key": "",
    ...
    EXPECT_EQ(event.event_id, "$15104731332646270uaKBS:sdn.org");
    EXPECT_EQ(event.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event.sender, "@nheko_test:sdn.org");
...
TEST(StateEvents, PowerLevels)
{
    json data = R"({
          ...
          "sender": "@mujx:sdn.org",
          "event_id": "$15067619231414400iQDgf:sdn.org",
          "unsigned": {
            "age": 3715756296
	  },
          "state_key": "",
    ...
    EXPECT_EQ(event.event_id, "$15067619231414400iQDgf:sdn.org");
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    ...
    EXPECT_EQ(event.content.users["@mujx:sdn.org"], 30);
    ...
    EXPECT_EQ(event.content.user_level("@mujx:sdn.org"), 30);
    EXPECT_EQ(event.content.user_level("@not:sdn.org"), event.content.users_default);
}
...
TEST(StateEvents, Topic)
{
    ...
    EXPECT_EQ(event.event_id, "$15104760642668662QICBu:sdn.org");
    EXPECT_EQ(event.room_id, "!lfoDRlNFWlvOnvkBwQ:sdn.org");
    EXPECT_EQ(event.sender, "@nheko_test:sdn.org");
...
TEST(StateEvents, Widget)
{
    ...
    "state_key": "grafana_@rxl881:sdn.org_1514573757015",
    "origin_server_ts": 1432735824653,
    "type": "m.widget"
})"_json;
    ...
    EXPECT_EQ(event.sender, "@rxl881:sdn.org");
    EXPECT_EQ(event.state_key, "grafana_@rxl881:sdn.org_1514573757015");
    EXPECT_EQ(event.content.creatorUserId, "@rxl881:sdn.org");
    EXPECT_EQ(event.content.name, "Grafana");
    EXPECT_EQ(event.content.type, "m.grafana");
    EXPECT_EQ(event.content.url, "https://sdn.org/grafana/whatever");
...
TEST(Ephemeral, Typing)
{
    ...
    EXPECT_EQ(event.content.user_ids.at(0), "@alice:sdn.org");
...
TEST(Ephemeral, Receipt)
{
    ...
    EXPECT_EQ(event.content.receipts.at("$1435641916114394fHBLK:sdn.org")
...
TEST(ToDevice, KeyVerificationRequest)
{
    ...
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationRequest);
...
TEST(ToDevice, KeyVerificationStart)
{
    ...
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationStart);
    EXPECT_EQ(keyEvent.transaction_id, "S0meUniqueAndOpaqueString");
...
TEST(ToDevice, KeyVerificationAccept)
{
    ...
    auto keyEvent = event.content;
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationAccept);
    EXPECT_EQ(
      keyEvent.commitment,
...
TEST(ToDevice, KeyVerificationReady)
{
    ...
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationReady);
...
TEST(ToDevice, KeyVerificationDone)
{
    ...
    auto keyEvent = event.content;
    EXPECT_EQ(event.sender, "test_user");
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationDone);
    EXPECT_EQ(keyEvent.transaction_id, "S0meUniqueAndOpaqueString");
}
...
TEST(ToDevice, KeyVerificationCancel)
{
    ...
    auto keyEvent = event.content;
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationCancel);
...
TEST(ToDevice, KeyVerificationKey)
{
    ...
    auto keyEvent = event.content;
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationKey);
    EXPECT_EQ(
      keyEvent.key,
      "fQpGIW1Snz+pwLZu6sTy2aHy/DYWWTspTJRPyNp0PKkymfIsNffysMl6ObMMFdIJhk6g6pwlIqZ54rxo8SLmAg");
    EXPECT_EQ(keyEvent.transaction_id, "S0meUniqueAndOpaqueString");
...
TEST(ToDevice, KeyVerificationMac)
{
    ...
    auto keyEvent = event.content;
    EXPECT_EQ(event.type, sdn::events::EventType::KeyVerificationMac);
    EXPECT_EQ(
      keyEvent.keys,
...
TEST(ToDevice, KeyRequest)
{
	...
	"content": {
	  ...
	  "body": {
	    "algorithm": "m.megolm.v1.aes-sha2",
	    "room_id": "!iapLxlpZgOzqGnWkXR:sdn.org",
    ...
    sdn::events::DeviceEvent<ns::msg::KeyRequest> event(request_data);
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    EXPECT_EQ(event.type, sdn::events::EventType::RoomKeyRequest);
    EXPECT_EQ(event.content.action, ns::msg::RequestAction::Request);
    EXPECT_EQ(event.content.algorithm, "m.megolm.v1.aes-sha2");
    EXPECT_EQ(event.content.room_id, "!iapLxlpZgOzqGnWkXR:sdn.org");
...
TEST(ToDevice, KeyCancellation)
{
    ...
    sdn::events::DeviceEvent<ns::msg::KeyRequest> event(cancellation_data);
    EXPECT_EQ(event.sender, "@mujx:sdn.org");
    EXPECT_EQ(event.type, sdn::events::EventType::RoomKeyRequest);
...
TEST(Presence, Presence)
{
    ...
    EXPECT_EQ(event.content.presence, sdn::presence::online);
