...
sdn::http::Client::put(const std::string &endpoint,
                       ...
                       bool requires_auth)
{
    sdn::http::Client::put<Request, sdn::responses::Empty>(
      endpoint,
      req,
      [callback](const sdn::responses::Empty, RequestErr err) { callback(err); },
      requires_auth);
}
...
template<class Response>
sdn::http::TypeErasedCallback
sdn::http::Client::prepare_callback(HeadersCallback<Response> callback)
{
    auto type_erased_cb = [callback](HeaderFields headers,
        ...
        auto invoke_callback = [&callback, &response_data, &headers](
                                 std::optional<sdn::http::ClientError> &&err) {
            try {
                callback(std::move(response_data), headers, std::move(err));
            } catch (std::exception &e) {
                sdn::utils::log::log()->critical("Application bug, exception escaped callback: {}",
                                                 e.what());
            }
        };
        ...
        if (status_code < 200 || status_code >= 300) {
            ...
            try {
                ...
                // fall through, if this is not a regular sdn response with a http error.
            }
...
sdn::http::Client::send_to_device(
  const std::string &txid,
  const std::map<sdn::identifiers::User, std::map<std::string, EventContent>> &messages,
  ErrCallback callback)
{
    constexpr auto event_type = sdn::events::to_device_content_to_type<EventContent>;
    static_assert(event_type != sdn::events::EventType::Unsupported);
    ...
    send_to_device(sdn::events::to_string(event_type), txid, j, callback);
}

template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::send_room_message(const std::string &room_id,
...
sdn::http::Client::send_room_message(const std::string &room_id,
                                     ...
                                     Callback<sdn::responses::EventId> callback)
{
    constexpr auto event_type = sdn::events::message_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);

    const auto api_path = "/client/v3/rooms/" + sdn::client::utils::url_encode(room_id) + "/send/" +
                          sdn::events::to_string(event_type) + "/" +
                          sdn::client::utils::url_encode(txn_id);

    put<Payload, sdn::responses::EventId>(api_path, payload, callback);
}

template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::send_state_event(const std::string &room_id,
                                    const std::string &state_key,
                                    const Payload &payload,
                                    Callback<sdn::responses::EventId> callback)
{
    constexpr auto event_type = sdn::events::state_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);

    const auto api_path = "/client/v3/rooms/" + sdn::client::utils::url_encode(room_id) +
                          "/state/" + sdn::events::to_string(event_type) + "/" +
...
void
sdn::http::Client::get_state(const std::string &room_id, Callback<sdn::responses::StateEvents> cb)
{
    const auto api_path = "/client/v3/rooms/" + sdn::client::utils::url_encode(room_id) + "/state";

    get<sdn::responses::StateEvents>(api_path,
                                     [cb = std::move(cb)](const sdn::responses::StateEvents &res,
                                                          HeaderFields,
...
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::get_state_event(const std::string &room_id,
                                   const std::string &type,
                                   const std::string &state_key,
                                   Callback<Payload> cb)
{
    const auto api_path = "/client/v3/rooms/" + sdn::client::utils::url_encode(room_id) +
                          "/state/" + sdn::client::utils::url_encode(type) + "/" +
                          sdn::client::utils::url_encode(state_key);
                                   ...
                                   Callback<Payload> cb)
{
    ...
    get_state_event<Payload>(room_id, sdn::events::to_string(event_type), state_key, std::move(cb));
}

template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::put_room_account_data(const std::string &room_id,
                                         const std::string &type,
                                         const Payload &payload,
                                         ErrCallback cb)
{
    const auto api_path = "/client/v3/user/" +
                          sdn::client::utils::url_encode(user_id_.to_string()) + "/rooms/" +
                          sdn::client::utils::url_encode(room_id) + "/account_data/" + type;
...
sdn::http::Client::put_room_account_data(const std::string &room_id,
                                         ...
                                         ErrCallback cb)
{
    constexpr auto event_type = sdn::events::account_data_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);
    put_room_account_data(room_id, to_string(event_type), payload, std::move(cb));
}
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::put_account_data(const std::string &type, const Payload &payload, ErrCallback cb)
{
    const auto api_path = "/client/v3/user/" +
                          sdn::client::utils::url_encode(user_id_.to_string()) + "/account_data/" +
                          type;
    put<Payload>(api_path, payload, cb);
}
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::put_account_data(const Payload &payload, ErrCallback cb)
{
    constexpr auto event_type = sdn::events::account_data_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);
...
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::get_room_account_data(const std::string &room_id,
                                         const std::string &type,
                                         Callback<Payload> cb)
{
    const auto api_path = "/client/v3/user/" +
...
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::get_room_account_data(const std::string &room_id, Callback<Payload> cb)
{
    constexpr auto event_type = sdn::events::account_data_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);
...
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::get_account_data(const std::string &type, Callback<Payload> cb)
{
    const auto api_path = "/client/v3/user/" +
                          sdn::client::utils::url_encode(user_id_.to_string()) + "/account_data/" +
                          type;
...
template<class Payload>
[[gnu::used, gnu::retain]] void
sdn::http::Client::get_account_data(Callback<Payload> cb)
{
    constexpr auto event_type = sdn::events::account_data_content_to_type<Payload>;
    static_assert(event_type != sdn::events::EventType::Unsupported);
