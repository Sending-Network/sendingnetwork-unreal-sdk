...
namespace common = sdn::common;

namespace sdn {
namespace responses {
//! Response from the `GET /_api/client/r0/directory/list/room/{roomId}`endpoint.
...
struct PublicRoomsChunk
{
    ...
    sdn::events::state::JoinRule join_rule = sdn::events::state::JoinRule::Public;
    ...
    std::vector<sdn::events::collections::StrippedEvents> children_state;

    friend void from_json(const nlohmann::json &obj, PublicRoomsChunk &res);
};
using PublicRoom = PublicRoomsChunk;

//! Response from the `GET /_api/client/r0/publicRooms` &
//! `POST /_api/client/r0/publicRooms` endpoints.
struct PublicRooms
{
...
//! Response from the `GET /_api/client/v1/rooms/{roomId}/hierarchy`
