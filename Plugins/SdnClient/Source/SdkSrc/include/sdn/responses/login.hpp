...
//! Response from the `POST /_api/client/r0/login` endpoint.
struct Login
{
    //! The fully-qualified user ID that has been registered.
    sdn::identifiers::User user_id;
...
{
    //! Optional UI hint for what kind of common SSO provider is being described in this IdP. SDN
    //! maintains a registry of identifiers in the sdn-doc repo to ensure clients and servers are
...
struct LoginFlow
{
    ...
    sdn::user_interactive::AuthType type;
...
//! Response from the `GET /_api/client/r0/login` endpoint.
struct LoginFlows
{
...
struct QueryDID
{
    ...
    friend void from_json(const nlohmann::json &obj, QueryDID &response);
};
...
struct PreLogin
{
    ...
    friend void from_json(const nlohmann::json &obj, PreLogin &response);
};
}
}
