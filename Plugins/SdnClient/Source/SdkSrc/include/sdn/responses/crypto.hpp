...
#include "sdn/lightweight_error.hpp"
...
namespace sdn {
namespace responses {
//! Response from the `POST /_api/client/r0/keys/upload` endpoint.
struct UploadKeys
{
...
using DeviceToKeysMap = std::map<std::string, sdn::crypto::DeviceKeys>;

//! Response from the `POST /_api/client/r0/keys/query` endpoint.
struct QueryKeys
{
    ...
    std::map<std::string, sdn::crypto::CrossSigningKeys> master_keys;
    //! A map from user ID, to information about user_signing_keys.
    std::map<std::string, sdn::crypto::CrossSigningKeys> user_signing_keys;
    //! A map from user ID, to information about self_signing_keys.
    std::map<std::string, sdn::crypto::CrossSigningKeys> self_signing_keys;
...
//! Request for `POST /_api/client/r0/keys/upload`.
struct KeySignaturesUpload
{
    //! Errors returned during upload.
    std::map<std::string, std::map<std::string, sdn::errors::LightweightError>> errors;

    friend void from_json(const nlohmann::json &obj, KeySignaturesUpload &response);
};

//! Response from the `POST /_api/client/r0/keys/claim` endpoint.
struct ClaimKeys
{
    //! If any remote homeservers could not be reached, they are
    //! recorded here. The names of the properties are the names
...
//! Response from the `GET /_api/client/r0/keys/changes` endpoint.
struct KeyChanges
{
    //! The User IDs of all users who updated their device identity keys.
    std::vector<std::string> changed;
    //! The User IDs of all users who may have left all the end-to-end
...
//! Responses from the `GET /_api/client/r0/room_keys/keys/{room_id}/{session_id}` endpoint
...
//! Responses from the `GET /_api/client/r0/room_keys/keys/{room_id}` endpoint
...
//! Responses from the `GET /_api/client/r0/room_keys/keys` endpoint
struct KeysBackup
{
...
//! Responses from the `GET /_api/client/r0/room_keys/version` endpoint
