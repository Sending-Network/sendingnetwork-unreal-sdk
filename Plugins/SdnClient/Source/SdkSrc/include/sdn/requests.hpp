...
/// @file
/// @brief Structs for for requests to the SDN API.
...
//! Request payload for the `POST /_api/client/r0/createRoom` endpoint.
struct CreateRoom
{
...
struct User
{
    //! A client can identify a user using their user ID. This can either be the fully
    //! qualified user ID, or just the localpart of the user ID.
    std::string user;
};
...
//! The user is identified by a DID.
struct DID
{
    std::string did;
    std::string address;
...
struct PreLogin
{
    std::string did;
    std::string address;
    friend void to_json(nlohmann::json &obj, const PreLogin &request);
};

//! Request payload for the `POST /_api/client/r0/login` endpoint.
struct Login
{
    //! The login type being used. One of ["m.login.password", "m.login.token", "m.login.did.identity"]
    std::string type = "m.login.did.identity";
    //! Identification information for the user. Usually you use `login_identifier::DID` with
    //! an did. Required.
    std::
      variant<login_identifier::User, login_identifier::Thirdparty, login_identifier::PhoneNumber, login_identifier::DID>
        identifier;
...
/// `POST /_api/client/r0/{register,account/password}/email/requestToken`
///
...
/// @brief Request payload for
/// `POST /_api/client/r0/{register,account/password}/msisdn/requestToken`
///
/// The homeserver should validate the email itself, either by sending a validation email itself or
/// by using a service it has control over.
...
//! Request payload for the `PUT /_api/client/r0/sendToDevice/{eventType}/{transcationID}`
template<typename EventContent>
using ToDeviceMessages = std::map<sdn::identifiers::User, std::map<std::string, EventContent>>;

//! Request payload for the `POST /_api/client/r0/profile/{userId}/avatar_url` endpoint.
struct AvatarUrl
{
...
//! Request payload for the `PUT /_api/client/r0/profile/{userId}/displayname` endpoint.
struct DisplayName
{
...
//! Request payload for the `POST /_api/client/r0/rooms/{roomId}/invite` endpoint as well as ban,
//! unban and kick.
...
//! Request payload for the `PUT /_api/client/r0/rooms/{roomId}/typing/{userId}` endpoint.
...
//! Request payload for the `PUT /_api/client/r0/devices/{deviceId}` endpoint.
struct DeviceUpdate
{
...
//! Request payload for the `PUT /_api/client/r0/directory/list/room/{roomId}` endpoint
...
//! Request payload for the `POST /_api/client/r0/publicRooms` endpoint.
struct PublicRooms
{
...
struct UploadKeys
{
    ...
    sdn::crypto::DeviceKeys device_keys;
...
struct KeySignaturesUpload
{
    ...
    std::map<
      std::string,
      std::map<std::string, std::variant<sdn::crypto::DeviceKeys, sdn::crypto::CrossSigningKeys>>>
      signatures;
...
struct DeviceSigningUpload
{
    ...
    std::optional<sdn::crypto::CrossSigningKeys> master_key;
    ...
    std::optional<sdn::crypto::CrossSigningKeys> self_signing_key;
    //! Optional. The user's user-signing key. Must be signed by the accompanying master key, or by
    //! the user's most recently uploaded master key if no master key is included in the request.
    std::optional<sdn::crypto::CrossSigningKeys> user_signing_key;
...
struct PusherData
{
    ...
    //! MUST be an HTTPS URL with a path of /_api/push/v1/notify.
    std::string url;
...
//! Request payload for the `POST /_api/client/r0/pushers/set` endpoint.
struct SetPusher
{
