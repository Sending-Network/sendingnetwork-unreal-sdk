...
/*! \brief Register for an account on this node.
 *
 ...
 * 
 * Register for an account on this node.
 * 
 ...
 * ``username`` provided, if any. Note that the grammar of SDN User ID
 * localparts is restricted, so the server MUST either map the provided
 * ``username`` onto a ``user_id`` in a logical manner, or reject
 * ``username``\s which do not comply to the grammar, with
 * ``M_INVALID_USERNAME``.
 * 
 * SDN clients MUST NOT assume that localpart of the registered
...
/// The fully-qualified SDN user ID that has been registered.
/// 
/// Any user ID returned by this API must conform to the grammar given in the
/// `SDN specification <../appendices.html#user-identifiers>`_.
 ...
 *
    ...
    * 
    * \param username
    *   The basis for the localpart of the desired SDN ID. If omitted,
    *   the node MUST generate a SDN ID local part.
    * 
    * \param password
 ...
 * an active session.
 * 
 * The node may change the flows available depending on whether a
 * valid access token is provided. The node SHOULD NOT revoke the
 ...
 * 
 * The node should validate the email itself, either by sending a
 ...
 * ``M_THREEPID_IN_USE`` may not be returned.
 * 
 * The node should validate the phone number itself, either by sending a
...
/// due to the identity server refusing the request or the node
/// being unable to determine an identity server to unbind from. This
/// must be ``success`` if the node has no identifiers to unbind
/// for the user.
 ...
 *
    ...
    *   If not provided, the node MUST use the ``id_server``
    *   that was originally use to bind each identifier. If the
    *   node does not know which ``id_server`` that was,
 ...
 * 
 * SDN clients may wish to use this API prior to attempting registration,
